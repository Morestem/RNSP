...
print("Dfx[current_temperature]:", df_x["current_temperature"].head(5))
print("Dfx[thermostat_status]:", df_x["thermostat_status"].head(5))
#df_x['current_temperature_ma'] = df_x['current_temperature'].rolling(50).mean().fillna(0)
#df_x['thermostat_status_ma'] = df_x['thermostat_status'].rolling(70).mean().fillna(0)
...
#df_test_x['current_temperature_ma'] = df_test_x['current_temperature'].rolling(50).mean().fillna(0)
#df_test_x['thermostat_status_ma'] = df_test_x['thermostat_status'].rolling(70).mean().fillna(0)
...
def encode(data, ths):
    ...
    for sample in data:
        ...
        temp_encoded = ths[0].binarize(sample[0])

        vec = np.append(vec, temp_encoded)

        # =========================================
        # thermostat_status -> binarização direta
        # =========================================

        status_bit = bool(sample[1] > 0.5)

        vec = np.append(vec, status_bit)
...
for bits_encoding in bits_list:
    ...
    for tuple_bit in tuple_list:
        ...
        estimated_entry_size = bits_encoding + 1
